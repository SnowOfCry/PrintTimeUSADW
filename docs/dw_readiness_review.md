...
| **Control/audit table naming** | ❌ **Inconsistent across the repo.** Bronze docs/comments reference `control.etl_batch_control`; the new audit DDL defines `audit.etl_batch_control` (`sql/audit/002`); the Docker init (`docker/postgres/init/002_create_control_tables.sql`) defines `control.elt_batch_log` + `control.elt_watermark`. Three different identities for "the batch control table." Pick one home (recommend `audit.etl_batch_control`) and update bronze comments + init. |
...
7. **Unify the control/audit table identity:** one of `control.etl_batch_control` (bronze docs) / `audit.etl_batch_control` (new DDL) / `control.elt_batch_log` (Docker init). Recommend standardizing on `audit.etl_batch_control` and updating the others.
