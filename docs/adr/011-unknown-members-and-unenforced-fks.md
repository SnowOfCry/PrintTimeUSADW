# ADR-011: `-1` Unknown Members and Unenforced Foreign Keys on Facts
...
**No `FOREIGN KEY` constraints on gold facts — indexed logical keys only. Every dimension seeds a reserved `-1` "Unknown" member, and any failed lookup resolves to `-1`, never `NULL`.**

- Each dimension gets one reserved row: key `-1`, text attributes `'Unknown'`, valid SCD2 housekeeping (`is_current = TRUE`), seeded **before the first fact load** (step 2 of the gold load order, ADR-007).
...
- Totals always reconcile: an unmatched product never subtracts from revenue; it shows up as product "Unknown" — visible in reports and honest about itself.
- No blank rows or dropped joins in Power BI / Tableau.
- Data-quality monitoring becomes trivial: unknown-member counts per fact per batch are a dashboardable metric.
...
- The `-1` seed rows are structural, not business data — dictionaries note them, and BI users occasionally ask what "Unknown" means (a documentation duty, ADR-013).
...
- `docs/naming_conventions/PrintTimeUSA_DW_Gold_Naming_Conventions.md` — `'Unknown'` / `-1` default conventions
- `docs/load_strategy/gold_load_strategy.md` — Unknown members section
