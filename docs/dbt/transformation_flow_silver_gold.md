...
flowchart TD
  ...
  classDef side fill:#eef,stroke:#88a
```
**Config (Type 2 dims):** `materialized='incremental'`, `incremental_strategy='append'`,
post-hooks to (1) close superseded versions and (2) seed `-1`. Append — not merge — because
merge would overwrite the prior row and collapse Type 2 into Type 1. Surrogate keys are
`GENERATED BY DEFAULT AS IDENTITY` so they stay stable across runs (facts reference them) and the
`-1` seed inserts cleanly. *See ADR-015.*
