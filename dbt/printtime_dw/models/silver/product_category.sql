...
-- Grain  : one row per category (business key: silver_category_id)
-- Purpose: clean product-category lookup; supplies the category name/description
--          and the department link that gold.dim_product rolls up to.
-- Spec   : sql/silver/002_create_silver_tables.sql (silver.product_category)
...
{{ config(
    ...
    unique_key='silver_category_id',
    incremental_strategy='merge',
    merge_exclude_columns=['silver_created_at_timestamp'],
    on_schema_change='fail'
) }}

-- 1) Pull from bronze. On incremental runs, only read batches not yet processed
--    (watermark = the highest bronze batch already landed in silver).
with source as (
    select * from {{ source('bronze', 'oltp_product_category') }}
    {% if is_incremental() %}
        where bronze_batch_id > (select coalesce(max(silver_bronze_batch_id), 0) from {{ this }})
    {% endif %}
),

-- 2) Bronze is append-only, so a category can appear several times. Keep only
...
-- 3) Cast to the DDL types and standardize. The code keeps its source case;
--    name/description are trimmed and internal spaces collapsed so equal values
--    become identical strings (stable hash, no phantom "changes").
cleaned as (
    select
        -- ── business columns ────────────────────────────────────────────────
        ...
        category_id::text                          as silver_source_record_id,
...
left join {{ this }} existing
    on existing.silver_category_id = f.silver_category_id
where existing.silver_category_id is null
