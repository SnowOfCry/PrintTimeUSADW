...
-- date views (vw_). Surrogate keys = <entity>_key (GENERATED ALWAYS AS
-- IDENTITY); natural/business keys = <entity>_id / *_code / *_number.
...
CREATE TABLE IF NOT EXISTS gold.dim_cashier (
    cashier_key INTEGER GENERATED ALWAYS AS IDENTITY,
...
COMMENT ON COLUMN gold.dim_cashier.cashier_key IS 'Surrogate key (identity).';
...
COMMENT ON COLUMN gold.dim_product.product_key IS 'Surrogate key (identity).';
...
COMMENT ON COLUMN gold.dim_customer.customer_key IS 'Surrogate key (identity).';
...
COMMENT ON COLUMN gold.dim_store.store_key IS 'Surrogate key (identity).';
...
CREATE TABLE IF NOT EXISTS gold.dim_invoice (
    invoice_key INTEGER GENERATED ALWAYS AS IDENTITY,
...
COMMENT ON COLUMN gold.dim_invoice.invoice_key IS 'Surrogate key (identity).';
...
COMMENT ON COLUMN gold.dim_payment_method.payment_method_key IS 'Surrogate key (identity).';
...
CREATE TABLE IF NOT EXISTS gold.dim_payment_type (
    payment_type_key INTEGER GENERATED ALWAYS AS IDENTITY,
...
COMMENT ON COLUMN gold.dim_payment_type.payment_type_key IS 'Surrogate key (identity).';
...
CREATE TABLE IF NOT EXISTS gold.fact_retail_sales (
    sales_line_key INTEGER GENERATED ALWAYS AS IDENTITY,
    date_key INTEGER,
    cashier_key INTEGER,
    product_key INTEGER,
...
COMMENT ON COLUMN gold.fact_retail_sales.sales_line_key IS 'Surrogate key (identity).';
...
CREATE TABLE IF NOT EXISTS gold.fact_payments (
    payment_key INTEGER GENERATED ALWAYS AS IDENTITY,
    invoice_key INTEGER,
    customer_key INTEGER,
...
COMMENT ON COLUMN gold.fact_payments.payment_key IS 'Surrogate key (identity).';
...
CREATE TABLE IF NOT EXISTS gold.fact_customer_behavior_snapshot (
    snapshot_key INTEGER GENERATED ALWAYS AS IDENTITY,
...
COMMENT ON COLUMN gold.fact_customer_behavior_snapshot.snapshot_key IS 'Surrogate key (identity).';
