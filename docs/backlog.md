...
| 3 | `dim_customer.customer_county` source | No county column exists in OLTP/bronze/silver; gold defaults to `'Unknown'`. Readiness review; planned ADR-014. | Business asks for county-level reporting → add ZIP→county reference table | ~half day |
...
| 8 | Operational runbook + deployment doc + freshness SLA | No documented deploy order, backfill/reprocess procedure, backup schedule, or "data ready by HH:MM" SLA. Docs audit; ADR-002 mitigation. | Before production go-live | ~half day |
