...
**Seeding mechanism.** The `-1` member has no Silver source row, so it is inserted by an idempotent dbt **post-hook** on each dim (`INSERT … VALUES (-1, 'Not Provided', …) ON CONFLICT (<key>) DO NOTHING`): the first run seeds it, later runs no-op. The surrogate keys are `GENERATED BY DEFAULT AS IDENTITY` (not `GENERATED ALWAYS`) specifically so the fixed `-1` can be inserted without `OVERRIDING SYSTEM VALUE`, while real rows still auto-generate positive keys (`1, 2, 3, …`) that never collide with `-1`.
