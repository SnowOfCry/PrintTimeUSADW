...
- Surrogate keys are dbt-managed plain `INTEGER`s (decision #7), so their correctness lives in the model (preserve existing keys from `{{ this }}`; new rows = `max(key) + running count`) rather than in a DB identity. The `-1` "Not Provided" member is a literal `UNION ALL` row, not an explicit identity insert — a convention to keep uniform across the six dims.
...
- `sql/gold/002_create_gold_tables.sql` — the dimension DDL (surrogate keys are plain `INTEGER`, assigned by dbt; the `GENERATED … AS IDENTITY` clause was removed per decision #7)
